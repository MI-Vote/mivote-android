...
/**
 * This code is largely taken from from the JetNews app in compose samples
...
import com.fueledbycaffeine.mivote.utils.getMutableStateOf

/**
 * Screen names (used for serialization)
 */
enum class ScreenName { BALLOT_STATUS }

/**
 * Class defining the screens we have in the app: home, article details and interests
 */
sealed class Screen(val id: ScreenName) {
    object BallotStatus : Screen(ScreenName.BALLOT_STATUS)
}
...
private fun Bundle.toScreen(): Screen {
    ...
    return when (screenName) {
        ScreenName.BALLOT_STATUS -> Screen.BallotStatus
    }
}
...
class NavigationViewModel(private val savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    @MainThread
    fun onBack(): Boolean {
        val wasHandled = currentScreen != Screen.BallotStatus
